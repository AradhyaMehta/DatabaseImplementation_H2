...
import org.h2.table.IncludeColumn;
...
public abstract class BaseIndex extends SchemaObjectBase implements Index {
    ...
    protected IncludeColumn[] includeColumns;
    protected Column[] columns;
    protected Column[] iColumns;
    protected int[] columnIds;
    protected int[] iColumnIds;
     ...
     *            not yet known
     * @param newIncludeColumns the columns that are included or null if this is
     *            not known
     * @param newIndexType the index type
     */
    protected void initBaseIndex(Table newTable, int id, String name,
            IndexColumn[] newIndexColumns, IndexType newIndexType, IncludeColumn[] newIncludeColumns) {
        ...
        if(newIncludeColumns != null) {
        	this.includeColumns = newIncludeColumns;
        	iColumns = new Column[newIncludeColumns.length];
            int len = iColumns.length;
            iColumnIds = new int[len];
            for (int i = 0; i < len; i++) {
                Column col = newIncludeColumns[i].column;
                iColumns[i] = col;
                iColumnIds[i] = col.getColumnId();
            }
        }
    }
    ...
    public int getiColumnIndex(Column col) {
    	if(iColumns != null) {
            for (int i = 0, len = iColumns.length; i < len; i++) {
                if (iColumns[i].equals(col)) {
                    return i;
                }
            }
    	}
    ...
    public IncludeColumn[] getIncludeColumns() {
