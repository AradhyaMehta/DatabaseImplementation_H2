...
public class IndexCursor implements Cursor {
    ...
    public void setIndex(Index index) {
        ...
        includeColumns = new IncludeColumn[columns.length];
        IncludeColumn[] idxiCols = index.getIncludeColumns();
        if (idxiCols != null) {
            for (int i = 0, len = columns.length; i < len; i++) {
                int idx = index.getiColumnIndex(columns[i]);
                if (idx >= 0) {
                    includeColumns[i] = idxiCols[idx];
                }
            }
        }
    }
