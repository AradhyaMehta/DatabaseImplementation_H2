...
public class PageBtreeIndex extends PageIndex {
    ...
    public PageBtreeIndex(RegularTable table, int id, String indexName,
            IndexColumn[] columns,
            IndexType indexType, boolean create, Session session, IncludeColumn[] iColumns) {
        initBaseIndex(table, id, indexName, columns, indexType, iColumns);
    ...
    private SearchRow getSearchRow(Row row) {
        SearchRow r = table.getTemplateSimpleRow(columns.length == 1 && iColumns == null);
    ...
    SearchRow readRow(Data data, int offset, boolean onlyPosition,
        ...
        synchronized (data) {
            ...
            SearchRow row = table.getTemplateSimpleRow(columns.length == 1 && iColumns != null);
            row.setKey(key);
