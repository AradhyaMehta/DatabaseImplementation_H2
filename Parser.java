...
public class Parser {
	...
	/**
	 * This method reads the include column list specified by the person
	 * creating the index.
	 * 
	 * @return an array of include columns
	 */
	private IncludeColumn[] parseIncludeColumnList() {
		ArrayList<IncludeColumn> columns = New.arrayList();
		do {
		...
		read(")");
		return columns.toArray(new IncludeColumn[columns.size()]);
	...
	private long readLong() {
		...
		if (currentTokenType != VALUE
				|| (currentValue.getType() != Value.INT && currentValue
						.getType() != Value.LONG)) {
	...
	private Prepared parseCreate() {
		...
		if (readIf("LOCAL")) {
			...
			if (readIf("INCLUDE")) {
				read("(");
				command.setIncludeColumns(parseIncludeColumnList());
			}
			return command;
		}
	}
