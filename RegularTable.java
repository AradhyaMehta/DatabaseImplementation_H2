...
public class RegularTable extends TableBase {
    ...
    public RegularTable(CreateTableData data) {
        ...
        if (data.persistData && database.isPersistent()) {
            mainIndex = new PageDataIndex(this, data.id,
                    ...
                    data.create, data.session, IncludeColumn.wrap(getColumns()));
            scanIndex = mainIndex;
        } else {
            mainIndex = null;
            scanIndex = new ScanIndex(this, data.id,
                    IndexColumn.wrap(getColumns()), IndexType.createScan(data.persistData), IncludeColumn.wrap(columns));
        }
    ...
    public Index addIndex(Session session, String indexName, int indexId,
            ...
            String indexComment, IncludeColumn[] iCols) {
        ...
        if (isPersistIndexes() && indexType.isPersistent()) {
            ...
            if (mainIndexColumn != -1) {
                ...
                index = new SpatialTreeIndex(this, indexId, indexName, cols,
                        indexType, true, create, session, iCols);
            } else {
                index = new PageBtreeIndex(this, indexId, indexName, cols,
                        indexType, create, session, iCols);
            }
        } else {
            if (indexType.isHash()) {
                if (cols.length != 1) {
                ...
                if (indexType.isUnique()) {
                    ...
                    index = new NonUniqueHashIndex(this, indexId, indexName,
                            cols, indexType, iCols);
                }
            } else if (indexType.isSpatial()) {
                index = new SpatialTreeIndex(this, indexId, indexName, cols,
                        indexType, false, true, session, iCols);
            } else {
                index = new TreeIndex(this, indexId, indexName, cols, indexType, iCols);
            }
        }
        if (database.isMultiVersion()) {
