...
public abstract class Value {
	...
	private static SoftReference<Value[]> softCache = new SoftReference<Value[]>(
			null);
	private static final BigDecimal MAX_LONG_DECIMAL = BigDecimal
			.valueOf(Long.MAX_VALUE);
	private static final BigDecimal MIN_LONG_DECIMAL = BigDecimal
	...
	/**
	 * And a value and return the result.
	 * 
	 * @param v
	 *            the value to and
	 * @return the result
	...
	/**
     * Or a value and return the result.
     *
	...
	/**
     * Xor a value and return the result.
     *
	...
	public Value convertTo(int targetType) {
		...
		try {
			...
			case BYTE: {
				...
				case DOUBLE:
					return ValueByte
							.get(convertToByte(convertToLong(getDouble())));
				case FLOAT:
					return ValueByte
							.get(convertToByte(convertToLong(getFloat())));
				case BYTES:
					return ValueByte.get((byte) Integer.parseInt(getString(),
							16));
				}
				break;
			}
			case SHORT: {
				...
				case BOOLEAN:
					return ValueShort
							.get(getBoolean().booleanValue() ? (short) 1
									: (short) 0);
				case BYTE:
				...
				case BYTES:
					return ValueShort.get((short) Integer.parseInt(getString(),
							16));
				}
				break;
			}
			case INT: {
				...
				case DOUBLE:
					return ValueInt
							.get(convertToInt(convertToLong(getDouble())));
				case FLOAT:
					return ValueInt
							.get(convertToInt(convertToLong(getFloat())));
				case BYTES:
			...
			case DATE: {
				...
				case TIME:
					...
					return ValueDate.fromDateValue(DateTimeUtils.dateValue(
							1970, 1, 1));
				case TIMESTAMP:
					return ValueDate.fromDateValue(((ValueTimestamp) this)
							.getDateValue());
				}
				break;
			}
			case TIME: {
				...
				case TIMESTAMP:
					return ValueTime.fromNanos(((ValueTimestamp) this)
							.getNanos());
				}
				break;
			}
			...
			case BYTES: {
				...
				case SHORT: {
					int x = getShort();
					return ValueBytes.getNoCopy(new byte[] { (byte) (x >> 8),
							(byte) x });
				}
				case INT: {
					int x = getInt();
			...
			case BLOB: {
				switch (getType()) {
				case BYTES:
					return ValueLobDb.createSmallLob(Value.BLOB,
							getBytesNoCopy());
				}
				break;
			}
			...
			case BYTES:
				return ValueBytes.getNoCopy(StringUtils.convertHexToBytes(s
						.trim()));
			case JAVA_OBJECT:
			...
			case CLOB:
				return ValueLobDb.createSmallLob(CLOB,
						s.getBytes(Constants.UTF8));
			case BLOB:
				return ValueLobDb.createSmallLob(BLOB,
						StringUtils.convertHexToBytes(s.trim()));
			case ARRAY:
	...
	/**
	 * Return the data handler for the values that support it (actually only
	 * Java objects).
	 * 
